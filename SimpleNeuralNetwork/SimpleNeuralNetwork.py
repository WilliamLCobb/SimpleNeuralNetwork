...
class NeuralNetwork(object):
    ...
    def __init__(self, nodes, new = False):
        #nodes is a ist containing the number of neurons in each layer
        self.numLayers = len(nodes)
        self.nodes = nodes
        fileAppendage = '-'.join([str(x) for x in nodes])
        self.fp = (os.path.dirname(os.path.realpath(__file__)) + "/networkData"+fileAppendage+".pkl")
        if new or not os.path.exists(self.fp): #No file exists so create a new network
            self.newNetwork(nodes)
        else:
            self.biases, self.weights = pickle.load(open(self.fp, 'r'))
    ...
    def saveNetwork(self):
        pickle.dump([self.biases, self.weights], open(self.fp, 'w'))

    def feedForward(self, a):
        # Return the outpt of the network if a is input.
    ...
    def train(self, trainingData, batchSize, eta, test_data, batchTests = True):
        ...
        for batch in batches:
            #batch[n][0] is image data, batch[n][1] is expected result
    ...
    def costDerivative(self, output_activations, y):
        ...
        #This is the main function to train the Netwrok
        #There are many of them so this may change
